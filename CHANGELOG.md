...
- TypeScript 脚本运行修复：兜底内联转译器仅在 typescript 模块存在时启用；依赖兼容映射固定 `typescript@5.6.3`，规避 TypeScript 7 原生发行版缺少 CJS `transpileModule` API 导致的必然失败。

### 环境变量

- 新建与改名接口增加名称校验：拒绝空名称、非法格式（须为字母/数字/下划线且不以数字开头）与运行时保留名，防止无效变量破坏备份等功能。

### 任务

- 创建任务时拒绝空命令，与前端表单校验保持一致。

### 依赖

- `POST /api/deps` 请求缺少 `names` 数组时返回 400 与契约提示，不再静默返回空结果。
...
- 取消正在安装或卸载的依赖时会终止对应进程，不再只改状态、让安装结束后把状态覆盖回成功或失败。系统包安装同样可以取消。
...
- 已通过 x86_64 模拟器运行时验证。ARM64 真机验证仍缺少自托管 runner，不以真机证据宣称 ARM64 已覆盖。
...
- 脚本执行流式输出改为 `io.Writer` 回收并配合 `WaitDelay` 兜底，修复 `Wait` 与管道读取并发导致的尾部日志丢失与数据竞争（内联脚本、订阅 hook 共用同一实现）。
- 定时任务全局调度器/执行器改用原子指针访问，消除并发读写的竞态。
- JWT 密钥生成移动到数据目录解析之后，并补齐生成失败的错误处理。
- 通知发送计数改为原子 `UPDATE` 自增，消除读-改-写竞态；Webhook 出站安全校验下沉到拨号层按解析结果 IP 校验，消除 DNS 重绑定 TOCTOU 窗口。
- CORS 拒绝日志改为有界存储避免内存无限增长，并停止放行 `Origin: null`。

### CLI 与运行时契约

- `ddp env set` 仅更新显式传入的字段，新增 `--enabled`/`--disabled`，不再清空未传的分组、备注和启用状态。
- 任务等待补充默认超时与超时报错，日志读取改为保留末尾并按需截断，只读命令不再触发迁移。
- 运行时契约校验补齐顶层 `artifacts` 字段，修复 x86_64 冒烟证据在严格反序列化下解析失败。
...
- 修复批量设置分组时已有分组无法取消选中的问题。
- 补齐各对话框临时 `TextEditingController` 的释放，消除内存泄漏。
